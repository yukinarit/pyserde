...
使い分けの目安として、単発のフィールドはフィールドシリアライザ、再利用したい型はクラスシリアライザ、プロジェクト全体に適用したい場合はグローバルシリアライザが向いています。
...
!!! tip
    `serde.field` を独自のフィールド関数でラップすると以下のようになります。
...
!!! tip
    クラス（デ）シリアライザはいくつでも登録できます。これは、好きなだけpyserde拡張を使用できることを意味します。
