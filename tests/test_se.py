...
from serde.core import TO_TUPLE
...
def test_render_primitives() -> None:
    rendered = Renderer(TO_TUPLE).render(SeField(int, "i"))
    assert rendered == 'coerce_object("None", "i", int, i)'


def test_render_list() -> None:

    rendered = Renderer(TO_TUPLE).render(SeField(list[int], "l"))
    assert rendered == '[coerce_object("None", "v", int, v) for v in l]'

    rendered = Renderer(TO_TUPLE).render(SeField(list[Foo], "foo"))
    assert rendered == f"[v.__serde__.funcs['to_tuple'](v, {kwargs}) for v in foo]"


def test_render_dict() -> None:
    rendered = Renderer(TO_TUPLE).render(SeField(dict[str, Foo], "foo"))
    rendered_key = 'coerce_object("None", "k", str, k)'
    rendered_val = f"v.__serde__.funcs['to_tuple'](v, {kwargs})"
    assert rendered == f"{{{rendered_key}: {rendered_val} for k, v in foo.items()}}"

    rendered = Renderer(TO_TUPLE).render(SeField(dict[Foo, Foo], "foo"))
    rendered_key = f"k.__serde__.funcs['to_tuple'](k, {kwargs})"
    rendered_val = f"v.__serde__.funcs['to_tuple'](v, {kwargs})"
    assert rendered == f"{{{rendered_key}: {rendered_val} for k, v in foo.items()}}"


def test_render_tuple() -> None:
    rendered = Renderer(TO_TUPLE).render(SeField(tuple[str, Foo, int], "foo"))
    rendered_str = 'coerce_object("None", "foo[0]", str, foo[0])'
    rendered_foo = f"foo[1].__serde__.funcs['to_tuple'](foo[1], {kwargs})"
...
def test_render_dataclass() -> None:
    rendered = Renderer(TO_TUPLE).render(SeField(Foo, "foo"))
    rendered_foo = f"foo.__serde__.funcs['to_tuple'](foo, {kwargs})"
